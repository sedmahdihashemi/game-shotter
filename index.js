...
_airplan.addEventListener('mouseup', (event) => {
    ...
    // Reset the position of the airplane element
    // _airplan.style.position = 'static';

    // Optionally, you can update the final position here
    x = event.clientX;
    y = event.clientY;
...
window.addEventListener('keypress', (e) => {
    ...
    if (e.keyCode === 32) {
        ...
        const newTir = document.createElement('span');
        newTir.classList.add('tir', 'bg-rose-500', 'w-3', 'h-3', '-top-2', 'left-[50%]', 'transform', 'translate-x-[-50%]', 'rounded-lg', 'absolute');
        // console.log(newTir);
        _airplan.appendChild(newTir);
        // console.log(_airplan);
        // ذخیره موقعیت اولیه هواپیما
        const initialAirplanePosition = {
            x: _airplan.offsetLeft,
        ...
        newTir.style.left = `${tirInitialPosition.x - 5}px`;
        ...
        }, 10); // فاصله زمانی بین هر حرکت را می‌توانید تنظیم کنید
